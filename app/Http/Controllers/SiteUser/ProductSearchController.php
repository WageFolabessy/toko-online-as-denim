...
class ProductSearchController extends Controller
{
    ...
    public function search(Request $request): ProductsPaginatedResource|JsonResponse
    {
        ...
        $cachedPaginator = Cache::get($cacheKey);

        if ($cachedPaginator instanceof LengthAwarePaginator) {
            Log::info('Cache hit for product search.', ['key' => $cacheKey]);
            return new ProductsPaginatedResource($cachedPaginator);
        }

        Log::info('Cache miss for product search. Proceeding with query.', ['key' => $cacheKey]);
        ...
        if ($paginator instanceof LengthAwarePaginator) {
            ...
            try {
                Cache::put($cacheKey, $paginator, now()->addMinutes(self::CACHE_TTL_MINUTES));
                Log::info('Stored search results in cache.', ['key' => $cacheKey, 'ttl_minutes' => self::CACHE_TTL_MINUTES]);
            } catch (\Exception $cacheError) {
                Log::error('Failed to store search results in cache.', ['key' => $cacheKey, 'error' => $cacheError->getMessage()]);
            }
    ...
    private function generateCacheKey(Request $request): string
    {
        ...
        return 'product_search:' . md5($queryString);
    }
    ...
    private function buildElasticsearchParams(Request $request): array
    {
        ...
        $allowedSortFields = ['updated_at', 'created_at', 'original_price', 'sale_price', 'product_name.keyword'];
